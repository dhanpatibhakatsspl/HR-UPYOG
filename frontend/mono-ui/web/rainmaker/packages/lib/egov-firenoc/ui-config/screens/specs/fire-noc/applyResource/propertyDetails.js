...
exports.propertyDetails = exports.updateUsageType = exports.triggerUpdateByKey = undefined;
...
var triggerUpdateByKey = exports.triggerUpdateByKey = function triggerUpdateByKey(state, keyIndex, value, dispatch) {
  if (dispatch == "set") {
    (0, _set2.default)(state, "screenConfiguration.preparedFinalObject.DynamicMdms.firenoc.buildings.selectedValues[" + keyIndex + "]", value);
  } else {
    dispatch((0, _actions.prepareFinalObject)("DynamicMdms.firenoc.buildings." + keyIndex, value));
  }
};
...
var prepareSelectField = function prepareSelectField(uom, start, end) {
  var data = [];
...
var updateUsageType = exports.updateUsageType = function () {
        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _loop = function _loop(i) {
              var subUsageType = (0, _get2.default)(buildings[i], "usageType", '') || '';
              var usageType = subUsageType && subUsageType.split('.')[0];
              var formObj = { buildingUsageType: usageType, buildingSubUsageType: subUsageType };
              triggerUpdateByKey(state, i, formObj, 'set');
              triggerUpdateByKey(state, "buildingSubUsageTypeTransformed.allDropdown[" + i + "]", (0, _commons.getObjectValues)((0, _get2.default)(state, "screenConfiguration.preparedFinalObject.DynamicMdms.firenoc.buildings.buildingsTransformed." + usageType, [])), dispatch);
              triggerUpdateByKey(state, "selectedValues[" + i + "]", formObj, dispatch);

              var cardType = (0, _get2.default)(state, "screenConfiguration.preparedFinalObject.FireNOCs[0].fireNOCDetails.noOfBuildings", null);
              var path = "";
              if (cardType === "SINGLE") {
              ...
              uoms.forEach(function (uom) {
                if (checkUomIsDefault(uom)) {
                  ...
                  dispatch((0, _actions.handleScreenConfigurationFieldChange)("apply", path, uom, dynamic(uom, path, i)));
                }
              });
  ...
  return function updateUsageType(_x, _x2) {
    return _ref.apply(this, arguments);
  };
}();

var buildingUsageTypeChange = function buildingUsageTypeChange(reqObj) {
  var dispatch = reqObj.dispatch,
      state = reqObj.state,
      value = reqObj.value,
      index = reqObj.index;

  dispatch((0, _actions.prepareFinalObject)("FireNOCs[0].fireNOCDetails.buildings[" + index + "].usageTypeMajor", value ? value : "none"));
};

var buildingSubUsageTypeChange = function buildingSubUsageTypeChange(reqObj) {
  var dispatch = reqObj.dispatch,
      state = reqObj.state,
      value = reqObj.value,
      index = reqObj.index;

  dispatch((0, _actions.prepareFinalObject)("FireNOCs[0].fireNOCDetails.buildings[" + index + "].usageType", value));
  var cardType = (0, _get2.default)(state, "screenConfiguration.preparedFinalObject.FireNOCs[0].fireNOCDetails.noOfBuildings", null);
  var path = "";
  if (cardType === "SINGLE") {
    path = "components.div.children.formwizardSecondStep.children.propertyDetails.children.cardContent.children.propertyDetailsConatiner.children.buildingDataCard.children.singleBuildingContainer.children.singleBuilding.children.cardContent.children.singleBuildingCard.children";
  ...
  uoms.forEach(function (uom) {
    if (checkUomIsDefault(uom)) {
      ...
      dispatch((0, _actions.handleScreenConfigurationFieldChange)("apply", path, uom, dynamic(uom, path, index)));
    }
  });
