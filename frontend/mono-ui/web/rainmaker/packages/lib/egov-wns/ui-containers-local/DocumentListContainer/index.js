...
var mapStateToProps = function mapStateToProps(state) {
  ...
  if (documentsList && documentsList.length > 0) {
    documentsList.forEach(function (document) {
      document.cards.forEach(function (card) {
        if (disabledCheck) card.disabled = true;else card.disabled = false;
      });
    });
  }
  var documents = (0, _get2.default)(state.screenConfiguration.preparedFinalObject, "applyScreen.documents", []);
  var uploadedDocuments = (0, _get2.default)(state.screenConfiguration.preparedFinalObject, "displayDocs");
  return { documentsList: documentsList, uploadedDocuments: uploadedDocuments, documents: documents };
};
