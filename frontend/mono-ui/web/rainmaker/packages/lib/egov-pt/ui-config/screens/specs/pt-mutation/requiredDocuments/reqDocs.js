...
var generateDocument = function generateDocument(item) {
  ...
  var subHeader = item.code && (0, _utils.getCommonTitle)({
    labelKey: (0, _commons.getTransformedLocale)("PT_" + item.code + "_HEADING")
  }, {
  ...
  if (item.hasOwnProperty("dropdownData")) {
    item.dropdownData = item.dropdownData.filter(function (item) {
      return item.active;
    });
    docs = item.dropdownData.reduce(function (obj, doc) {
      obj[doc.code] = (0, _utils2.getLabelOnlyValue)({
        labelKey: (0, _commons.getTransformedLocale)("PT_" + doc.code + "_LABEL")
      }, {
        style: styles.docs
      });
      return obj;
    ...
    item.options = item.options.filter(function (item) {
      return item.active;
    });
  ...
  var subParagraph = item.description ? (0, _utils.getCommonParagraph)({
    labelKey: (0, _commons.getTransformedLocale)("PT_" + item.description + "_NOTE")
  }, {
