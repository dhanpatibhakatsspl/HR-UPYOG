...
var validateField = exports.validateField = function validateField(field) {
  ...
  var value = fieldValue ? typeof fieldValue === "string" ? fieldValue.trim() : fieldValue : null;
  var errorText = "",
      isFieldValid = true,
      fieldLength = 0;
  ...
  if (required && !value) {
    ...
    errorText = field.requiredMessage || (0, _commons.getLocaleLabels)("REQUIRED_FIELD", "REQUIRED_FIELD");
  }
  ...
  if (isDOB) {
    if (value) {
      var currentDate = maxDate ? new Date(maxDate).getTime() : new Date().getTime();
