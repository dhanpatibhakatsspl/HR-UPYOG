...
@Results({ @Result(name = BankRemittanceAction.NEW, location = "dayEndRemittance-new.jsp"),
...
@ParentPackage("egov")
public class DayEndRemittanceVoucherAction extends GenericWorkFlowAction {
	...
	@Action(value = "/receipts/dayEndRemittance-newform")
	@SkipValidation
	...
	@Action(value = "/receipts/dayEndRemittance-listData")
	@SkipValidation
	public String listData() {
		...
		if (!hasErrors() && accountNumberId != null) {
			...
			final String fundCode = fundCodeList != null && !fundCodeList.isEmpty() ? fundCodeList.get(0).toString()
					: null;
			final CFinancialYear financialYear = financialYearDAO.getFinancialYearById(finYearId);
			receiptBeanList = remittanceService.findRemittanceDetailsForServiceAndFund("",
	...
	@Action(value = "/receipts/dayEndRemittance-create")
...
//                voucher();
		...
		remittedReceiptList = remittanceService.createCashAndChequeBankRemittance(finalBeanList, accountNumberId,
				remittanceDate, getInstrumentIdArray(), workflowBean);
	...
	private void populateNames(List<Receipt> receiptList) {
		...
		for (Receipt r : receiptList) {
			if (r.getBill().get(0).getBillDetails().get(0).getBusinessService() != null
				...
				r.getBill().get(0).getBillDetails().get(0).setBusinessService(businessDetailsCodeNameMap
						.get(r.getBill().get(0).getBillDetails().get(0).getBusinessService()));
		}
	}
