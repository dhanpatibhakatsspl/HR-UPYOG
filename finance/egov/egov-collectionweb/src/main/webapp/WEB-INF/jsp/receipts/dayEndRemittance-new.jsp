...
<script type="text/javascript">
	...
	function validate() {
		...
		if (!isChecked(document.getElementsByName('receiptIds'))) {
			...
			return false;
				...
				document.dayEndRemittanceForm.action = "dayEndRemittance-create.action";
				jQuery(dayEndRemittanceForm).append(jQuery('<input>', {
		            type : 'hidden',
	...
	function searchDataToRemit() {
		...
		document.dayEndRemittanceForm.action = "dayEndRemittance-listData.action";
		jQuery(dayEndRemittanceForm).append(jQuery('<input>', {
            type : 'hidden',
            name : '${_csrf.parameterName}',
            value : '${_csrf.token}'
...
<body>
	...
	<s:form theme="simple" name="dayEndRemittanceForm">
		<s:push value="model">
			<s:token />
			<s:if test="%{hasErrors()}">
			...
			<div class="formmainbox">
				<div class="subheadnew">
					<s:text name="day.end.remittance.voucher.title" />
				</div>
				<div align="center">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td class="bluebox"><select id="accountNumberId"
								name="accountNumberId" value="%{accountNumberId}">
									<option value="-1">Select</option>
									<c:forEach items="${dropdownData.accountNumberList}"
										var="accNum">
										<c:if test="${accNum.bankAccount == accountNumberId }">
											<option value="${accNum.bankAccount}" selected="selected">${accNum.bank}
												- ${accNum.bankAccount}</option>
										</c:if>
										<c:if test="${accNum.bankAccount != accountNumberId }">
											<option value="${accNum.bankAccount}">${accNum.bank}
												- ${accNum.bankAccount}</option>
										</c:if>
									</c:forEach>
							</select></td>
				...
				<div class="buttonbottom">
					<input name="search" type="submit" class="buttonsubmit" id="search"
						value="<s:text name='lbl.search'/>"
						onclick="return searchDataToRemit()" />
				</div>
				<s:if test="%{!receiptBeanList.isEmpty()}">
					<display:table name="receiptBeanList" uid="currentRow"
						...
						<display:column headerClass="bluebgheadtd" class="blueborderfortd"
							...
							<input type='checkbox'
								name='finalBeanList[${rowNumber}].selected'
								id='selected_${rowNumber}' value="false"
								onClick="handleReceiptSelectionEvent()" />
							<input type="hidden" name="finalBeanList[${rowNumber}].service"
								id="service_${rowNumber}" value="${currentRow.service}" />
							<input type="hidden" name="finalBeanList[${rowNumber}].fund"
								id="fund_${rowNumber}" value="${currentRow.fund}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].department"
								id="department_${rowNumber}" value="${currentRow.department}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].instrumentAmount"
								id="instrumentAmount_${rowNumber}"
								value="${currentRow.instrumentAmount}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].instrumentNumber"
								id="instrumentNumber_${rowNumber}"
								value="${currentRow.instrumentNumber}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].instrumentDate"
								id="instrumentDate_${rowNumber}"
								value="${currentRow.instrumentDate}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].instrumentType"
								id="instrumentType_${rowNumber}"
								value="${currentRow.instrumentType}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].instrumentId"
								id="instrumentId_${rowNumber}"
								value="${currentRow.instrumentId}" />
							<input type="hidden" name="finalBeanList[${rowNumber}].bank"
								id="bank_${rowNumber}" value="${currentRow.bank}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].bankBranch"
								id="bankBranch_${rowNumber}" value="${currentRow.bankBranch}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].receiptNumber"
								id="receiptNumber_${rowNumber}"
								value="${currentRow.receiptNumber}" />
							<input type="hidden" name="finalBeanList[${rowNumber}].receiptId"
								id="receiptId_${rowNumber}" value="${currentRow.receiptId}" />
							<input type="hidden"
								name="finalBeanList[${rowNumber}].receiptDate"
						...
						<display:column headerClass="bluebgheadtd" class="blueborderfortd"
							...
							value="${currentRow.receiptNumber}" />
						<display:column headerClass="bluebgheadtd" class="blueborderfortd"
							title="Cheque/DD/Cash number and date"
							style="width:20%;text-align: center"
							value="${currentRow.instrumentNumber}  ${currentRow.instrumentDate}" />
						<%-- <display:column headerClass="bluebgheadtd" class="blueborderfortd"
							title="Drawee bank and branch"
							style="width:20%;text-align: center"
							value="${currentRow.bank}  ${currentRow.bankBranch}" /> --%>
						<display:column headerClass="bluebgheadtd" class="blueborderfortd"
						...
						<display:column headerClass="bluebgheadtd" class="blueborderfortd"
							title="Cheque/Cash/DD Amount (Rs)"
					...
					<input type="hidden" name="selectedRowsId" id="selectedRowsId"
					...
					<s:if test="%{receiptBeanList.isEmpty()}">
						...
						<div class="buttonbottom">
							<input name="buttonClose" type="button" class="button"
								id="buttonClose" value="<s:text name='lbl.close'/>"
