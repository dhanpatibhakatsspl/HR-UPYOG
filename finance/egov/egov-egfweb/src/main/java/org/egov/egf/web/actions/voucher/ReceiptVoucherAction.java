...
public class ReceiptVoucherAction extends BaseFormAction {
	...
	public void prepare() {
		...
		if (null != parameters.get("showMode")) {
			...
			if (showMode.equalsIgnoreCase("nonBillPayment")) {
				...
				List<String> onlyReceipt = VoucherHelper.VOUCHER_TYPES.stream().filter(type -> FinancialConstants.STANDARD_VOUCHER_TYPE_RECEIPT.equals(type))
						.collect(Collectors.toList());
				addDropdownData("typeList", onlyReceipt);
			}
	...
	@SkipValidation
	@Action(value = "/voucher/receiptSearch-beforesearch")
