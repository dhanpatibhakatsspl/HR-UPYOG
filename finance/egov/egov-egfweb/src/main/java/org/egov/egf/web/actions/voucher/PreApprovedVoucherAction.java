...
public class PreApprovedVoucherAction extends GenericWorkFlowAction {
    ...
    public String list() {
        ...
        if (isFieldMandatory("department"))
            preApprovedVoucherList = getPersistenceService().findAllBy(
        ...
        if (!action.equalsIgnoreCase("save")) {
            if (getModel().getId() != null) {
                if (getModel().getCurrentState() != null) {
                    ...
                    wfMatrix = this.customizedWorkFlowService.getWfMatrix(getModel().getStateType(),
                            getWorkFlowDepartment(), getAmountRule(), getAdditionalRule(),
                            State.DEFAULT_STATE_VALUE_CREATED, getPendingActions(), getModel().getCreatedDate());
                }
            }
        } else {
            CVoucherHeader model = new CVoucherHeader();
            if (model.getId() != null) {
                if (model.getCurrentState() != null) {
                    wfMatrix = this.customizedWorkFlowService.getWfMatrix(model.getStateType(), getWorkFlowDepartment(),
                            getAmountRule(), getAdditionalRule(), model.getCurrentState().getValue(),
                            getPendingActions(), model.getCreatedDate());
                } else {
                    wfMatrix = this.customizedWorkFlowService.getWfMatrix(model.getStateType(), getWorkFlowDepartment(),
                            getAmountRule(), getAdditionalRule(), State.DEFAULT_STATE_VALUE_CREATED,
                            getPendingActions(), model.getCreatedDate());
                }
            }
        }
    ...
    public String approve() throws ApplicationException {
        ...
        if (voucherHeader.getType().equals(FinancialConstants.STANDARD_VOUCHER_TYPE_CONTRA)) {
            ...
            contraWorkflowService.transition(parameters.get(ACTIONNAME)[0], contraVoucher,
            ...
            voucherWorkflowService.transition(parameters.get(ACTIONNAME)[0], voucherHeader,
                    parameters.get("comments")[0]);
    ...
    private void addActionMsg(final String stateValue, final Long pos) {
        ...
        else if (parameters.get(ACTIONNAME)[0].contains("co_reject")
                || parameters.get(ACTIONNAME)[0].contains("aa_reject") || "END".equals(stateValue))
            addActionMessage(getText("voucher.cancelled"));
        else
            addActionMessage(
                    getText("pjv.voucher.rejected", new String[] { voucherService.getEmployeeNameForPositionId(pos) }));
        ...
        try {
            ...
            if (cutOffDate1 != null && voucherDate.compareTo(cutOffDate1) <= 0
                ...
                if (voucherHeader.getVouchermis().getBudgetaryAppnumber() == null) {
                    addActionMessage(
                            getText("Voucher created successfully. Voucher No : ") + voucherHeader.getVoucherNumber());
                } else {
                    addActionMessage(getText("Voucher created successfully. Voucher No : ")
                            + voucherHeader.getVoucherNumber() + " And " + getText("budget.recheck.sucessful",
    ...
    private void validateBillVoucherDate(EgBillregister egBillregister, CVoucherHeader voucherHeader) {
        // Allow bills accounting accross years (appacofig key
        // :allow_billsaccounting_across_years and values are 'yes' and 'no')
        AppConfig appConfigAccrossYears = appConfigService.getAppConfigByModuleNameAndKeyName(
                FinancialConstants.MODULE_NAME_APPCONFIG, FinancialConstants.APPCONFIG_BILLACCOUNTING_ACCROSS_YEARS);
        ...
        try {
            if ("no".equalsIgnoreCase(appConfigAccrossYears.getConfValues().get(0).getValue()) && !financialYearDAO
                    .isSameFinancialYear(egBillregister.getBilldate(), voucherHeader.getVoucherDate())) {
                throw new ValidationException("bill.accounting.year.check", "bill.accounting.year.check");
            }
            ...
            if ("yes".equalsIgnoreCase(appConfigAccrossYears.getConfValues().get(0).getValue())
                ...
                throw new ValidationException("bill.accounting.voucherdate.enddate.check",
                        getText("bill.accounting.voucherdate.enddate.check",
    ...
    public void sendForApproval() {
        ...
        // voucherWorkflowService.transition(parameters.get(ACTIONNAME)[0] + "|"
        // + userId, voucherHeader,
        // parameters.get("comments")[0]);
        voucherService.persist(voucherHeader);
    }
    ...
    public String getSourcePath() {
        String sourcePath;
        if (voucherHeader != null && voucherHeader.getGeneralledger() != null
                && voucherHeader.getGeneralledger().size() > 0)
    ...
    public Map<String, Object> getMasterName() throws ApplicationException {
        ...
        for (final CGeneralLedger gl : gllist) {
            if (gl.getFunctionId() != null)
                names.put(gl.getGlcodeId().getGlcode(), ((CFunction) getPersistenceService()
                        .find(" from CFunction where id=?", Long.valueOf(gl.getFunctionId()))).getName());

            // get subledger.
            final List<CGeneralLedgerDetail> gldetailList = getPersistenceService()
                    .findAllBy("from CGeneralLedgerDetail where generalLedgerId.id=?", gl.getId());
    ...
    public void getMasterDataForBillVoucher() throws ApplicationException {
        ...
        if (voucherHeader != null) {
            ...
            for (final EgBilldetails billdetails : egBillregister.getEgBilldetailes()) {
                ...
                temp.put(Constants.GLCODE, coa.getGlcode());
                temp.put("accounthead", coa.getName());
                temp.put(Constants.DEBITAMOUNT,
                        billdetails.getDebitamount() == null ? 0 : billdetails.getDebitamount());
                temp.put(Constants.CREDITAMOUNT,
                        billdetails.getCreditamount() == null ? 0 : billdetails.getCreditamount());
                temp.put("billdetailid", billdetails.getId());
                tempList.add(temp);
    ...
    public String ajaxValidateDetailCode() {
        ...
        if (adt.getTablename().equalsIgnoreCase("EG_EMPLOYEE")) {
            final PersonalInformation information = (PersonalInformation) getPersistenceService()
                    .find(" from PersonalInformation where employeeCode=? and isActive=true", code);
            if (information == null)
                values = index + "~" + ERROR;
            else
                values = index + "~" + information.getIdPersonalInformation() + "~"
                        + information.getEmployeeFirstName();
        } else if (adt.getTablename().equalsIgnoreCase("RELATION")) {
            final Relation relation = (Relation) getPersistenceService()
                    .find(" from Relation where code=? and isactive=true", code);
            if (relation == null)
                values = index + "~" + ERROR;
            else
                values = index + "~" + relation.getId() + "~" + relation.getName();
        } else if (adt.getTablename().equalsIgnoreCase("ACCOUNTENTITYMASTER")) {
            final AccountEntity accountEntity = (AccountEntity) getPersistenceService()
                    .find(" from AccountEntity where code=? and isactive=true ", code);
    ...
    public void setJournalvouchermodify(final JournalVoucherModifyAction journalvouchermodify) {
