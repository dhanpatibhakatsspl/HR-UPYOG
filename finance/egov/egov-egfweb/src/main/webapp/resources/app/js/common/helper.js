...
function showAmountInWordsn(type) {
	var netamt = $('#' + type + '-netPayableAmount').val();
    const amount = netamt;
	//alert(netamt);
    const words = convertNumberToWords(amount);
    var amountInWordsn = "In Words :- " + words+ " Ruppes Only /- "
    document.getElementById('amountInWordsn').innerText = amountInWordsn || '';
}
function convertNumberToWords(num) {
    ...
    const scales = [
        { value: 1e17, name: 'Shankha' },
        { value: 1e15, name: 'Padam' },
    ...
    function numToWords(n) {
        ...
        for (let i = 0; i < scales.length; i++) {
            if (n >= scales[i].value) {
                const scaleCount = Math.floor(n / scales[i].value);
                const remainder = n % scales[i].value;
                str = numToWords(scaleCount) + ' ' + scales[i].name;
                if (remainder) {
                    str += ' ' + numToWords(remainder);
                }
                return str;
            }
        }
